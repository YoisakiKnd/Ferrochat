<script lang="ts">
	...
	const initChatList = async () => {
		...
		try {
			...
			let firstPage = [];
			if (search) {
				firstPage = await getChatListBySearchText(localStorage.token, search, $currentChatPage);
			} else {
				firstPage = await getChatList(localStorage.token, $currentChatPage);
			}
			allChatsLoaded = !firstPage || firstPage.length < 60;
			await chats.set(firstPage ?? []);
		} catch (error) {
			console.error(error);
			allChatsLoaded = true;
			if (!$chats) {
	...
	const loadMoreChats = async () => {
		...
		try {
			...
			allChatsLoaded = !newChatList || newChatList.length < 60;
