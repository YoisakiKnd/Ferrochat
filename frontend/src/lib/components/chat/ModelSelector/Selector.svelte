		...
		<slot>
			...
			<div class="px-3 max-h-64 overflow-y-auto scrollbar-hidden group relative">
				...
				{#each filteredItems as item, index}
					{#if item.model?.provider?.name && item.model.provider.name !== filteredItems[index - 1]?.model?.provider?.name}
						<div class="px-3 pt-2 text-xs font-medium text-gray-400">{item.model.provider.name}</div>
					{/if}
					<button
