<script>
	...
	const executePythonAsWorker = async (id, code, cb) => {
		if (cb) {
			cb({ stdout: null, stderr: 'Python execution is disabled in Ferrochat', result: null });
		}
		return;
	};
				...
				if (done) {
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${title} | Ferrochat`, {
								body: content,
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isFocused) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${data?.user?.name} (#${event?.channel?.name}) | Ferrochat`, {
							body: data?.content,
	...
	onMount(async () => {
		...
		const localeChosen = localStorage.getItem('locale-chosen') === '1';
		if (!localeChosen) {
			localStorage.removeItem('locale');
		}
		initI18n(localeChosen ? localStorage.locale : undefined);
		if (!localeChosen) {
